...
def route_action(text: str, line_user_id: str | None) -> ActionRoute:
    ...
    if is_question_text(text) and route.should_mutate_db:
        if route.action in {"mark_payable_paid", "create_payable"} or get_payable_type(text):
            return finish(
                ...
                f"{base_route_source}+safety_guard",
                raw_model_output=raw_route_model_output,
                error=route_error,
            )
        return finish(
            ActionRoute(action="chat", should_mutate_db=False, confidence=0.70, reason="safety: question cannot mutate data"),
            f"{base_route_source}+safety_guard",
            raw_model_output=raw_route_model_output,
            error=route_error,
        )
    ...
    if route.action == "create_expense" and parse_amount(text) is None:
        return finish(
            ActionRoute(action="chat", should_mutate_db=False, confidence=0.70, reason="safety: expense without amount"),
            f"{base_route_source}+safety_guard",
            raw_model_output=raw_route_model_output,
            error=route_error,
        )
    ...
    if route.action == "create_payable":
        ...
        if not route.item_type or route.amount is None:
            return finish(
                ActionRoute(action="chat", should_mutate_db=False, confidence=0.65, reason="safety: incomplete payable"),
                f"{base_route_source}+safety_guard",
                raw_model_output=raw_route_model_output,
                error=route_error,
            )
...
def parse_expense_text_fallback(text: str) -> ExpenseEntry:
    ...
    specific_day = extract_expense_specific_day(text)
    if specific_day:
        target_date = date.fromisoformat(specific_day.start_date)
    elif "\u6628\u5929" in text:
    ...
    with get_db() as conn:
        conn.execute(
            ...
            ON CONFLICT(chat_id) DO UPDATE SET
                ...
                created_at = excluded.created_at
            """,
            (chat_id, actor_user_id, title, item_key, category, scheduled_date, scheduled_time, original_text, created_at),
        )
        conn.commit()
        ...
        with get_db() as conn:
            ...
            if len(reminded) == 1:
                ...
                if len(due_rows) == 1:
                    ...
                    return build_home_task_choice_reply(list(due_rows))
    if not candidates:
...
def find_expense_candidates(
    ...
    *,
    ...
    has_explicit_date = any(
        keyword in raw_text
        for keyword in ("\u4eca\u5929", "\u6628\u5929", "\u9019\u5169\u5929", "\u8fd1\u5169\u5929", "\u6628\u5929\u8ddf\u4eca\u5929", "\u6628\u5929\u548c\u4eca\u5929")
    ) or bool(
        re.search(r"\d{4}[-/]\d{1,2}[-/]\d{1,2}|\d{1,2}/\d{1,2}|\d{1,2}\u6708\d{1,2}[\u65e5\u865f]?", raw_text)
    )
    dates = get_query_dates(raw_text) if has_explicit_date else []
    ...
    params: list[str | int | None] = ["TWD", scope_line_user_id, scope_line_user_id]
    if dates:
        where.insert(0, f"date IN ({','.join('?' for _ in dates)})")
        params = [*dates, *params]
    if category:
        where.append("category = ?")
        params.append(category)
...
def delete_expense(raw_text: str, line_user_id: str | None) -> DeleteResult:
    ...
    with get_db() as conn:
        if is_delete_confirm and not id_match:
            ...
            row = conn.execute(
                ...
                WHERE id = ?
                  AND (? IS NULL OR line_user_id = ? OR line_user_id IS NULL)
                """,
                (pending_id, scope_line_user_id, scope_line_user_id),
            ).fetchone()
            if not row:
        ...
        if id_match and len(normalized_text) <= len(id_match.group(1)) + 3:
            ...
            row = conn.execute(
                ...
                WHERE id = ?
                  AND (? IS NULL OR line_user_id = ? OR line_user_id IS NULL)
                """,
                (expense_id, scope_line_user_id, scope_line_user_id),
            ).fetchone()
            if not row:
        ...
        row = conn.execute(
            ...
            LIMIT 1
            """,
                (target.date, target.amount, target.currency, target.category, scope_line_user_id, scope_line_user_id),
            ).fetchone()
...
def recent_sql_changes_reply(line_user_id: str | None, limit: int = 5) -> str:
    ...
    with get_db() as conn:
        rows = conn.execute(
            ...
            LIMIT ?
            """,
            (scope_line_user_id, scope_line_user_id, limit),
        ).fetchall()
    if not rows:
...
def apply_last_utterance_feedback(raw_text: str, actor_user_id: str | None, scope_id: str | None) -> str | None:
    match = re.search(
        "^\\s*\\u4e0a\\u4e00\\u53e5(?:\\u5224\\u932f\\uff0c?)?(?:\\u4e0d\\u662f\\u8a18\\u5e33\\uff0c?)?\\s*(?:\\u61c9\\u8a72\\u662f|\\u662f)\\s*(.+?)\\s*$",
        raw_text,
    )
    if not match:
        return None
    ...
    scope_actor_user_id = get_household_read_scope_line_user_id(actor_user_id)
    with get_db() as conn:
        row = conn.execute(
            """
            SELECT id
            ...
            LIMIT 1
            """,
            (scope_id, scope_id, scope_actor_user_id, scope_actor_user_id, raw_text),
        ).fetchone()
        if not row:
            return "我找不到可以修正的上一句記錄。"
        conn.execute(
            """
...
def expense_parse_debug_user_key(line_user_id: str | None) -> str:
    return line_user_id or "_anonymous"


def route_debug_user_key(line_user_id: str | None) -> str:
    return line_user_id or "_anonymous"
...
def save_last_route_debug(
    ...
    *,
    ...
    payload: dict[str, object] = {
        ...
        "final_route": final_route.model_dump(mode="json"),
    ...
    if error:
        payload["error"] = error
    last_route_debug_by_user[route_debug_user_key(line_user_id)] = payload


def route_debug_reply(line_user_id: str | None) -> str:
    debug_data = last_route_debug_by_user.get(route_debug_user_key(line_user_id))
    if not debug_data:
        return "\u9084\u6c92\u6709\u53ef\u4ee5\u6aa2\u8996\u7684 route \u5224\u65b7\u7d50\u679c\uff0c\u8acb\u5148\u50b3\u4e00\u53e5\u4e00\u822c\u8a0a\u606f\u518d\u7528 checkRoute\u3002"

    lines = [
        "\u6700\u8fd1\u4e00\u6b21 route \u5224\u65b7",
        f"original_text\uff1a{debug_data['original_text']}",
        f"route_source\uff1a{debug_data['route_source']}",
        f"raw_route_model_output\uff1a{debug_data.get('raw_model_output') or '(\u7121)'}",
        "final_route\uff1a" + json.dumps(debug_data["final_route"], ensure_ascii=False, sort_keys=True),
    ]
...
def parse_expense_candidate(normalized_text: str) -> ParsedExpenseCandidate:
    raw_model_output: str | None = None
    source = "openai"
    error_message: str | None = None
    try:
        expense = parse_expense_text(normalized_text)
        raw_model_output = (last_openai_expense_parse or {}).get("output")
    except OpenAIError as exc:
        logger.exception("OpenAI failed; using fallback parser for LINE text: %s", normalized_text)
        source = "fallback"
        error_message = str(exc)
        expense = parse_expense_text_fallback(normalized_text)
...
def build_expense_parse_debug_entry(candidate: ParsedExpenseCandidate) -> dict[str, object]:
    entry: dict[str, object] = {
        "normalized_text": candidate.normalized_text,
        "source": candidate.source,
        "final_payload": candidate.expense.model_dump(mode="json"),
    }
    if candidate.raw_model_output is not None:
        entry["raw_model_output"] = candidate.raw_model_output
    if candidate.error:
        entry["error"] = candidate.error
    return entry


def save_last_expense_parse_debug(
    line_user_id: str | None,
    original_text: str,
    entries: list[dict[str, object]],
) -> None:
    last_expense_parse_debug_by_user[expense_parse_debug_user_key(line_user_id)] = {
        "original_text": original_text,
        "entries": entries,
        "created_at": datetime.now(timezone.utc).isoformat(),
    }
...
def expense_gpt_debug_reply(line_user_id: str | None) -> str:
    debug_data = last_expense_parse_debug_by_user.get(expense_parse_debug_user_key(line_user_id))
    if not debug_data:
        return "\u9084\u6c92\u6709\u53ef\u4ee5\u6aa2\u8996\u7684 ChatGPT \u89e3\u6790\u7d50\u679c\uff0c\u8acb\u5148\u8a18\u4e00\u7b46\u5e33\u518d\u7528 checkGpt\u3002"

    lines = [
        "\u6700\u8fd1\u4e00\u6b21 ChatGPT \u89e3\u6790",
        f"original_text\uff1a{debug_data['original_text']}",
    ]
    for index, entry in enumerate(debug_data.get("entries", []), start=1):
        lines.append("")
        lines.append(f"\u7b2c {index} \u7b46")
        lines.append(f"normalized_text\uff1a{entry['normalized_text']}")
        lines.append(f"raw_model_output\uff1a{entry.get('raw_model_output') or '(\u7121)'}")
        if entry.get("source") and (entry.get("source") != "openai" or entry.get("error")):
            lines.append(f"source\uff1a{entry['source']}")
        if entry.get("error"):
            lines.append(f"error\uff1a{entry['error']}")
        lines.append("final_payload\uff1a" + json.dumps(entry["final_payload"], ensure_ascii=False, sort_keys=True))
    return "\n".join(lines)
...
def handle_special_command(raw_text: str, line_user_id: str | None) -> str | None:
    ...
    if raw_text.strip().lower() in {"checkgpt"}:
        return expense_gpt_debug_reply(line_user_id)
    if raw_text.strip().lower() in {"checkroute"}:
        return route_debug_reply(line_user_id)
...
def create_expense_reply(raw_text: str, line_user_id: str | None, message_id: str | None) -> str:
    candidate = parse_expense_candidate(raw_text)
    save_last_expense_parse_debug(
        line_user_id,
        raw_text,
        [build_expense_parse_debug_entry(candidate)],
    )
    expense_id = save_expense(candidate.expense, raw_text, line_user_id, message_id)
    return build_reply_text(candidate.expense, expense_id)
...
def create_expense_with_confirmation_reply(
    ...
    if parsed_amount is not None and parsed_amount > expense.amount:
        expense = expense.model_copy(update={"amount": parsed_amount})
        candidate = candidate.model_copy(update={"expense": expense})
...
def is_multiline_context_header(line: str) -> bool:
    if not re.search(r"\d+", line):
        return True
    has_date_marker = bool(re.search(r"\d{1,2}/\d{1,2}|\d{1,2}月\d{1,2}[日號]?", line)) or any(
        keyword in line for keyword in ("今天", "昨天", "前天")
    )
    has_count_marker = bool(re.search(r"(?:共)?[一二兩三四五六七八九十\d]+筆", line))
    ends_with_amount = bool(re.search(r"\d+\s*(?:元|塊)?\s*$", line))
    return (has_date_marker or has_count_marker) and not ends_with_amount


def normalize_multiline_shared_context(line: str) -> str:
    cleaned = re.sub(r"(?:共)?[一二兩三四五六七八九十\d]+筆", "", line)
    cleaned = re.sub(r"^(\d{1,2}/\d{1,2})(\S)", r"\1 \2", cleaned)
    cleaned = re.sub(r"^(\d{1,2}月\d{1,2}[日號]?)(\S)", r"\1 \2", cleaned)
    cleaned = re.sub(r"\s+", " ", cleaned).strip("：:，,、 ")
    return cleaned or line.strip()


def normalize_multiline_detail_line(line: str) -> str:
    cleaned = re.sub(r"^\s*(?:第?[一二兩三四五六七八九十\d]+筆)\s*", "", line)
    return cleaned.strip() or line.strip()


def semantic_parse_multi_expense(raw_text: str) -> MultiExpenseSemanticResult | None:
    lines = [line.strip() for line in raw_text.splitlines() if line.strip()]
    shared_context: str | None = None
    detail_lines = lines
    if len(lines) > 2 and is_multiline_context_header(lines[0]):
        shared_context = normalize_multiline_shared_context(lines[0])
        detail_lines = lines[1:]
    is_multiline_create = (
        len(detail_lines) > 1
        and all(re.search(r"\d+", line) for line in detail_lines)
        and not any(is_question_text(line) for line in lines)
    )
    if not is_multiline_create:
        return None

    semantic_entries: list[ParsedExpenseCandidate] = []
    for line in detail_lines:
        normalized_line = normalize_multiline_detail_line(line)
        normalized_text = f"{shared_context} {normalized_line}".strip() if shared_context else normalized_line
        detail_amount = parse_amount(normalized_line)
        candidate = parse_expense_candidate(normalized_text)
        expense = candidate.expense
        if detail_amount is not None and detail_amount > 0 and detail_amount != expense.amount:
            expense = expense.model_copy(update={"amount": detail_amount})
        if shared_context and expense.note:
            normalized_note = re.sub(rf"^{re.escape(shared_context)}\s*", "", expense.note).strip()
            expense = expense.model_copy(update={"note": normalized_note or expense.note})
    ...
    return MultiExpenseSemanticResult(
...
def build_db_expense_rows(
    semantic_result: MultiExpenseSemanticResult,
    message_id: str | None,
) -> list[ExpenseDbRowDraft]:
    rows: list[ExpenseDbRowDraft] = []
    for index, entry in enumerate(semantic_result.entries, start=1):
        rows.append(
            ExpenseDbRowDraft(
                raw_text=entry.normalized_text,
                message_id=f"{message_id}:{index}" if message_id else None,
...
def create_multiline_expense_reply(raw_text: str, line_user_id: str | None, message_id: str | None) -> str | None:
    semantic_result = semantic_parse_multi_expense(raw_text)
    if semantic_result is None:
        return None

    created_entries = []
    for row in build_db_expense_rows(semantic_result, message_id):
        save_expense(row.expense, row.raw_text, line_user_id, row.message_id)
        time_text = f" {row.expense.time}" if row.expense.time else ""
        created_entries.append(f"{row.expense.date}{time_text} {row.expense.note or row.expense.category} TWD {row.expense.amount}")
    save_last_expense_parse_debug(
        line_user_id,
        semantic_result.original_text,
        [build_expense_parse_debug_entry(entry) for entry in semantic_result.entries],
    )
